...
sub_model = tf.keras.models.load_model(r"models/model_sub")
top_model = tf.keras.models.load_model(r"models/model_top")
bottom_model = tf.keras.models.load_model(r"models/model_bottom")
foot_model = tf.keras.models.load_model(r"models/model_shoes")
...
def find_combo_by_top(top_color_group, combotype):
    """
    Greedy algorithm to recommend colors based on a seed color and a given angle in a colorwheel.
    Input is a color (from 12+3 colorwheel) and an angle: moderate_combo == 90
                                                         similar_combo == 60
                                                         close_combo == 30
                                                         same_combo == 0
    Output is a list of two colors.
    """
    co = int(combotype / 30)
    ...
    if top_color_group == 15:
        ...
        bottom_color_group = 13  # Choose white as a neutral color
    elif top_color_group in [12, 13, 14]:
        # If top color is mono
        bottom_color_group = 13 if top_color_group == 12 else 12
    else:
        # For other cases, choose a color within the specified angle
        bottom_color_group = random.choice([12, 13])
    
    # Choose shoes color
    shoes_color_group = (bottom_color_group + co) % 6
